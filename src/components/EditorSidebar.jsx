        ...
        {Array.from({ length: numPages }).map((_, idx) => (
          ...
          >
            <div className="relative w-full aspect-[1/1.4] bg-white border-2 border-slate-200 hover:border-blue-500 rounded-lg shadow-sm overflow-hidden flex items-center justify-center transition-colors">
              ...
              <button className="absolute bottom-1 right-1 w-6 h-6 bg-blue-500 rounded text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10 hover:bg-blue-600 shadow" title="Rotate Page" onClick={(e)=>{e.stopPropagation(); handlePageRotate,
  handleAddBlankPage(idx);}}>↻</button>
      ...
      <div className="p-4 border-t border-slate-200 bg-white">
        <button onClick={handleAddBlankPage} className="w-full py-2.5 border-2 border-dashed border-slate-300 rounded-xl text-sm font-semibold text-slate-600 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-colors flex items-center justify-center gap-2">
